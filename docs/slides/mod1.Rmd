---
title: "Module 1:  R, what is it good for?"
subtitle:  "Introduction and Overview"
author: "Graham Eglit"
institute: "Canopy Growth Corporation"
date: "May 20, 2022"
output:
  xaringan::moon_reader:
    css: 
      - xaringan-themer.css
    lib_dir: libs
    seal: false
    self_contained: false
    nature:
      highlightStyle: github
      highlightLines: true
      ratio: 16:9
---

```{r xaringan-themer, include=FALSE, warning=FALSE}
require(xaringanthemer)
require(xaringanExtra)
style_duo(primary_color = "#e3e3e2", secondary_color = "#a6873b")
```

```{r xaringan-panelset, echo=FALSE}
xaringanExtra::use_panelset()
```

class: title-slide, right, top
background-image: url(img/canopy_logo.png)
background-size: 50%, cover

.right-column[
# `r rmarkdown::metadata$title`
### `r rmarkdown::metadata$subtitle`

**`r rmarkdown::metadata$author`**<br>
`r rmarkdown::metadata$institute`<br>
`r rmarkdown::metadata$date`
]


---
background-image: url(img/canopy_logo.png)
background-size: 14%, cover
background-position: 99% 4%

# .center[Why R?]

.pull-left[
## R/ R Studio
<img src="img/hex_rstudio.png" width="30%"/>
- Free
- Open Source
- Large User Base/Community
- Cutting-Edge Statistics
- Reproducibility
- Version Control/Collaboration
]

.pull-right[
## R Markdown
<img src="img/hex_rmarkdown.png" width="30%"/>
- Reproducibility
- Communication
]

---
background-image: url(img/canopy_logo.png)
background-size: 14%, cover
background-position: 99% 4%

class: center

# Growth of R

.panelset[
.panel[.panel-name[Jobs - 2017 & 2019]
<img src="img/mod1/pop_jobs.png" width="40%"/>
.bottom[Figure from [The Popularity of Data Science Software](https://r4stats.com/articles/popularity/)]
] <!---->

.panel[.panel-name[Job Change 2017 - 2019]
<img src="img/mod1/pop_jobchange.png" width="36%"/>
.bottom[Figure from [The Popularity of Data Science Software](https://r4stats.com/articles/popularity/)]
] <!---->

.panel[.panel-name[Primary Tool]
<img src="img/mod1/pop_use.png" width="50%"/>
.bottom[Figure from [The Popularity of Data Science Software](https://r4stats.com/articles/popularity/)]
] <!---->

.panel[.panel-name[1995 - 2016 Articles]
<img src="img/mod1/pop_time.png" width="40%"/>
.bottom[Figure from [The Popularity of Data Science Software](https://r4stats.com/articles/popularity/)]
] <!---->

.panel[.panel-name[2018 Articles]
<img src="img/mod1/pop_articles.png" width="38%"/>
.bottom[Figure from [The Popularity of Data Science Software](https://r4stats.com/articles/popularity/)]
] <!---->

] <!--end of panelset-->


---
background-image: url(img/canopy_logo.png)
background-size: 14%, cover
background-position: 99% 4%

# .center[You Can Do All Sorts of Cool Stuff In R]

---
background-image: url(img/canopy_logo.png)
background-size: 14%, cover
background-position: 99% 4%

# .center[History of R]


- R is a descendent of the S programming language
- S was developed at the Bell Telephone Laboratories in 1976
- S was designed for data analysis, but with the goal that *users* would become *developers*
- 1988, S was rewritten in C and began to resemble the R language we use today.  
    - This was the third version of S (S3)
    - S4 was later released in 1998
- Eventually S was commercially released as S-Plus
- R was created in 1991 by Ross Ihaka and Robert Gentleman
- R was released using a GNU General Public License in 1995, ensuring that it would remain free and open source
- In 2001, R version 1.0.0 was released to the public
- The most recent version of R is 4.2.0, which was released on 2022-04-22
<br>
.center[<img src="img/mod1/gentleman.png" width="15%"/>
<img src="img/mod1/ihaka.jpg" width="20%"/>]