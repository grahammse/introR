...
ggplot(dat) + 
  geom_bar(mapping = aes(x = var, y = out),
           stat = "identity")
```
...
- To change positioning, use the `position` argument for `geom_bar`
- "fill" will retain stacking, but render stacked bars the same height
...
.center[
# Position Adjustment with `geom_point`
]
