...
- `read_*` functions guess the type of objects in a column
   - Inspect the first 1000 rows and makes a best guess <br> <br>
- These guesses are usually correct <br> <br>
- When they're wrong, we can use parsing functions to ensure that the appropriate column type is created <br> <br>
...
```{r echo = FALSE}
data.frame(
   Type = c("logical", "integer", "double", "number", "time", "date", "date-time", "character"),
   Heuristic = c("Contains only F, T, FALSE, or TRUE", "Contains only numeric characters", "Contains only valid doubles (including numbers like 4.5e-5", "Contains valid doubles with the grouping mark inside", "Matches the default time_format", "Matches the default date_format", "Any ISO8601 date", "Doesn't meet any criteria above")
) %>% 
   knitr::kable("html", booktabs = FALSE) %>% 
